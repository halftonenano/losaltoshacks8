<script lang="ts">
  ...
  let changed=false;
  let valuesOnly2: string[] = selectedUsers.map(user => user.value);
  ...
  async function handleUpdate() {
    ...
    const valuesOnly = selectedUsers.map(user => user.value);
    const data = {
      "users": valuesOnly
    };

    const record = await pb.collection('areas').update(id, data);
  }
  ...
  onMount(async () => {
    const record = await pb.collection('areas').getOne(id);
    const matchingUsers = $sync.users.filter(user => record.users.includes(user.id));

    // Separate list of matching users
    selectedUsers = matchingUsers.map(user => ({
      value: user.id,
      label: user.name
    }));
  })
